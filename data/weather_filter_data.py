...
BASE_DIR = "data/"
wea_path  = BASE_DIR + "WeatherEvents/WeatherEvents_Jan2016-Dec2022.csv"

OUT_DIR = os.path.join(BASE_DIR, "cities_filtered_weather")
...
# 4 city đại diện
# AirportCode dùng IATA phổ biến cho metro lõi
CITIES = {
    "new_york": {
        "state": "NY",
        "pattern": r"\bNew York\b",
        "airports": ["JFK", "LGA", "EWR", "NYC"]  # NYC metro core
    },
    "chicago": {
        "state": "IL",
        "pattern": r"\bChicago\b",
        "airports": ["ORD", "MDW", "CHI"]
    },
    "miami": {
        "state": "FL",
        "pattern": r"\bMiami\b",
        "airports": ["MIA", "FLL", "OPF"]  # Miami Intl, Fort Lauderdale, Opa-locka
    },
    "los_angeles": {
        "state": "CA",
        "pattern": r"\bLos Angeles\b",
        "airports": ["LAX", "BUR", "LGB", "ONT"]  # LAX + metro airports
    },
}

# output paths + xoá file cũ
out_paths = {}
first_write = {}
totals = {}

for key in CITIES:
    out_16_21 = os.path.join(OUT_DIR, f"{key}_weather_2016_2021.csv")
    out_22    = os.path.join(OUT_DIR, f"{key}_weather_2022.csv")
...
for chunk in pd.read_csv(wea_path, chunksize=CHUNK, low_memory=False):
    city_col = chunk["City"].astype(str).str.strip()
    airport_col = chunk["AirportCode"].astype(str).str.strip()
    ...
    for key, cfg in CITIES.items():
        ...
        # Filter: airport code thuộc metro HOẶC (State đúng & City đúng)
        sub = chunk[
            airport_col.isin(airports) |
            (
        ...
        df16, df22 = split_time(sub, "StartTime(UTC)", "EndTime(UTC)")
