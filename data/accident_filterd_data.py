...
BASE_DIR = "data/"
acc_path  = BASE_DIR + "accidents/US_Accidents_March23.csv"

OUT_DIR = os.path.join(BASE_DIR, "cities_filtered_accidents")
os.makedirs(OUT_DIR, exist_ok=True)

CHUNK = 200_000  # giảm xuống 50k-100k nếu vẫn thiếu RAM
...
for key in CITIES:
    out_16_21 = os.path.join(OUT_DIR, f"{key}_accidents_2016_2021.csv")
    out_22    = os.path.join(OUT_DIR, f"{key}_accidents_2022.csv")
...
for chunk in pd.read_csv(acc_path, chunksize=CHUNK, low_memory=False):
    city_col = chunk["City"].astype(str)

    # xử lý lần lượt từng city trong chunk
    for key, cfg in CITIES.items():
        ...
        df16, df22 = split_time(sub, "Start_Time", "End_Time")
